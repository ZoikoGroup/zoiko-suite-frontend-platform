...
// - wage-garnishment-svc (8095)
// - direct-deposit-svc (8096)
// - year-end-filing-svc (8097)
...
function wageGarnishmentUrl(): string {
  return (process.env.ZOIKO_WAGE_GARNISHMENT_URL ?? "http://localhost:8095").replace(/\/$/, "");
}
...
const MOCK_GARNISHMENTS: Garnishment[] = [
  {
    garnishment_id: "grn-001",
    tenant_id: "11111111-1111-1111-1111-111111111111",
    employee_id: "emp-104",
    legal_entity_id: "22222222-2222-2222-2222-222222222222",
    garnishment_type: "CHILD_SUPPORT",
    creditor_name: "State Family Services",
    ordered_amount: 500.0,
    withheld_to_date: 2500.0,
    currency: "USD",
    status: "ACTIVE",
    effective_from: "2026-01-01",
    created_at: "2026-01-01T00:00:00Z",
...
type GarnishmentsResponse = { garnishments: Garnishment[]; total: number };

export async function listGarnishments(identity?: Identity): Promise<ApiResult<Garnishment[]>> {
  const base = wageGarnishmentUrl();
  const url = `${base}/v1/garnishments`;
  return fetchServiceWithFallback<GarnishmentsResponse, Garnishment[]>(
    url, base, "wage-garnishment-svc", identity,
    (d) => d.garnishments ?? [], MOCK_GARNISHMENTS
  );
}

// ─── 7. Direct Deposit ───────────────────────────────────────────────────────

function directDepositUrl(): string {
  return (process.env.ZOIKO_DIRECT_DEPOSIT_URL ?? "http://localhost:8096").replace(/\/$/, "");
}

export type DirectDepositInstruction = {
...
const MOCK_DIRECT_DEPOSITS: DirectDepositInstruction[] = [
  {
    instruction_id: "dd-001",
    tenant_id: "11111111-1111-1111-1111-111111111111",
    employee_id: "emp-101",
...
type DirectDepositResponse = { instructions: DirectDepositInstruction[]; total: number };

export async function listDirectDepositInstructions(identity?: Identity): Promise<ApiResult<DirectDepositInstruction[]>> {
  const base = directDepositUrl();
  const url = `${base}/v1/direct-deposit`;
  return fetchServiceWithFallback<DirectDepositResponse, DirectDepositInstruction[]>(
    url, base, "direct-deposit-svc", identity,
    (d) => d.instructions ?? [], MOCK_DIRECT_DEPOSITS
  );
}

// ─── 8. Year-End Filings ─────────────────────────────────────────────────────

function yearEndFilingUrl(): string {
  return (process.env.ZOIKO_YEAR_END_FILING_URL ?? "http://localhost:8097").replace(/\/$/, "");
}

export type YearEndFiling = {
  filing_id: string;
  tenant_id: string;
  legal_entity_id: string;
  tax_year: number;
...
const MOCK_YEAR_END_FILINGS: YearEndFiling[] = [
  {
    filing_id: "yef-2025-us-w2",
    tenant_id: "11111111-1111-1111-1111-111111111111",
    legal_entity_id: "22222222-2222-2222-2222-222222222222",
    tax_year: 2025,
    form_type: "W-2",
    ...
    employee_count: 85,
    status: "SUBMITTED",
    submitted_at: "2026-01-31T17:00:00Z",
    created_at: "2026-01-15T09:00:00Z",
  },
];

type YearEndFilingsResponse = { filings: YearEndFiling[]; total: number };

export async function listYearEndFilings(identity?: Identity): Promise<ApiResult<YearEndFiling[]>> {
  const base = yearEndFilingUrl();
  const url = `${base}/v1/year-end-filings`;
  return fetchServiceWithFallback<YearEndFilingsResponse, YearEndFiling[]>(
    url, base, "year-end-filing-svc", identity,
    (d) => d.filings ?? [], MOCK_YEAR_END_FILINGS
  );
}
